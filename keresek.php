<?php
	...
	$lekerdez = mysqli_query($link, "SELECT * FROM film_keres $keres $kategoria ORDER BY $listazas DESC LIMIT $limit");
...
<div class="row">
  ...
  <div class="col-sm-8">
  <a href="keresek.php?listazas=evszam_szerint&keres=<?=$keres_cim?>&kategoria=<?=$kategoria_keres?>"><button type="button" class="btn btn-primary">Évszám szerint</button></a> <a href="keresek.php?keres=<?=$keres_cim?>&kategoria=<?=$kategoria_keres?>"><button type="button" class="btn btn-primary">Feltöltés szerint</button></a>
  ...
  <br />
	...
	if (isset($_GET['page'])) {
        ...
        $total_pages_sql = "SELECT COUNT(*) FROM film_keres $keres $kategoria";
        $result = mysqli_query($link,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM film_keres $keres $kategoria ORDER BY $listazas DESC LIMIT $offset, $no_of_records_per_page";
	...
	$film_teljesito = mysqli_fetch_array(mysqli_query($link, "SELECT * FROM felhasznalok WHERE username='".$row["teljesitette"]."'"));
	  ?>
	<div class="card border-dark">
	  <div class="card-header"><a href="keres_adatlap.php?id=<?=$row["id"]?>"><button type="button" class="btn btn-primary">Kérés adatlapja</button></a> <p class="pull-right">Kérési azonosító: <?=$row["id"]?></p></div>
	  <div class="card-body">
		...
		<p class="card-text">
			...
			<div class="row">
				<div class="col-sm-6">
					<b>Kérte:</b> <a href="profil.php?id=<?=$film_feltolto["id"]?>"><?=$film_feltolto["username"]?></a> <br />
					<b>Teljesítve: </b> <?php if ($row["allapot"] == "true") { echo "Igen, teljesítette: <a href='profil.php?id=".$film_teljesito["id"]."'>".$row["teljesitette"]."</a>"; } else { echo "Nem"; } ?>
				</div>
			</div>
		</p>
	  </div>
