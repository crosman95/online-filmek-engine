	...
	<div class="col-sm-2"></div>
		<div class="col-sm-8">
			<div class="card border-dark">
			  ...
			  <div class="card-body">
				<p class="card-text">
					Ez a film már megtalálható az adatbázisban!
				</p>
			  </div>
			</div>
		</div>
...
if ($_GET["feldolgoz"] == "film_bekuld") {
	...
	$kuldott_film_link = htmlspecialchars($_POST["film_link"]);
	
	if (!empty($kuldott_film_link)) {
	// API BEHÍVÁSA
	...
	if ($film_db_id_ellenorzes["db_id"] == "" AND $bekuldott_db_id_ellenorzes["db_id"] == "" AND $film_cim_ellenorzes["cim"] == "" AND $bekuldott_cim_ellenorzes["cim"] == "") {
		mysqli_query($link, "INSERT INTO `bekuldott_filmek`(`id`, `db_id`, `cim`, `eredeti_cim`, `leiras`, `megjelenes`, `hossza`, `ertekeles`, `kategoria`, `kep`, `film_link`, `felhasznalo`, `ellenorzes`, `keresre`) VALUES (NULL,'$db_id','$cim','$eredeti_cim','$leiras','$megjelenes','$hossza','$ertekeles','$kategoria','$kep','$domain','$felhasznalo','$ellenorzes','$keresre')");
	}
	else {
...
<div class="row">
	<div class="col-sm-2"></div>
		<div class="col-sm-8">
			<div class="card border-dark">
			  <div class="card-header">Film beküldve</div>
			  <div class="card-body">
				<p class="card-text">
...
<div class="row">
	<div class="col-sm-2"></div>
		<div class="col-sm-8">
			<div class="card border-dark">
			  <div class="card-header">Film beküldése</div>
			  <div class="card-body">
				<p class="card-text">
					<?php
					...
					<form method="POST" action="?feldolgoz=film_bekuld">
						<table width="100%px">
							...
							<tr>
								<td>Film link:</td>
								<td><input required class="form-control" type="text" value="" name="film_link" /></td>
							</tr>
							<tr>
								<td>Kérési azonosító:</td>
								<td><input class="form-control" placeholder="Csak akkor töltsd ki, ha kérést teljesítesz" type="text" value="" name="keresi_azonosito" /></td>
							</tr>
							<tr>
								<td></td>
								<td><input class="btn btn-success" type="submit" value="Feltöltés" /></td>
							</tr>
						</table>
					</form>
					...
					<form method="GET" action="film_bekuldes.php">
						<table>
							<tr>
